...
var softManager = {
    ...
    getList: function(param, callback) {
    	//获取数据
        var softwareTaskUrl = com_huawei_vcenterpluginui.webContextPath + "/rest/services/software/progress/list?s=" + Math.random();
        //通过API获取数据
        $.get(softwareTaskUrl, function(response){
    ...
    add: function(param, callback, failCallback) {
    	 var softwareSaveUrl = com_huawei_vcenterpluginui.webContextPath + "/rest/services/software/upload?s=" + Math.random();
         $.ajax({  
             type: 'POST',
    ...
    delete: function(param, callback) {
        //删除代码逻辑 根据param.softwareName
    	var deleteSoftwareUrl = com_huawei_vcenterpluginui.webContextPath + "/rest/services/software/"+param.param+"?ip="+param.esight+"&s=" + Math.random();
    	$.ajax({  
            type: 'DELETE',
            contentType : 'application/json;charset=utf-8',
            url: deleteSoftwareUrl,  
            dataType: "json",
            success: function(response){  
            	console.log(response);
                if (typeof callback === "function") {
    ...
    clear: function(param, callback) {
        // param = {}
    	//删除失败任务逻辑
    	var deleteFailTaskUrl = com_huawei_vcenterpluginui.webContextPath + "/rest/services/software?s=" + Math.random();
    	$.ajax({  
            type: 'DELETE',
            contentType : 'application/json;charset=utf-8',
