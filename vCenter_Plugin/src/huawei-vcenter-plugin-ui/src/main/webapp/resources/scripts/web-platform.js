...
var WEB_PLATFORM = self.parent.parent.WEB_PLATFORM;
if (!WEB_PLATFORM) {
   WEB_PLATFORM = self.parent.parent.document.getElementById("container_app");
   self.parent.parent.WEB_PLATFORM = WEB_PLATFORM;
...
for (var i = 0; i < scriptArray.length; i++) {
    var src = scriptArray[i].getAttribute("src");
    if(src != undefined) {
        scriptArray[i].setAttribute("src", here + src);
    }
}
...
for (var i = 0; i < linkArray.length; i++) {
    ...
    if(href != undefined) {
        linkArray[i].setAttribute("href", here + href);
    }
}
