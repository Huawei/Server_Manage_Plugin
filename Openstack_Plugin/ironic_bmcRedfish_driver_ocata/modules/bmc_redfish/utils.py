...
# key is uuid (unique one), value is conn
_CONN = {}
...
def get_system(node):
    ...
    :param node: an Ironic node object
    :raises: BMCRedfishConnectionError when it fails to connect to Redfish
    :raises: BMCRedfishError if the System is not registered in Redfish
    """
    driver_info = parse_driver_info(node)
    address = driver_info['address']
    system_id = driver_info['system_id']
    node_uuid = driver_info['node_uuid']

    @retrying.retry(
        retry_on_exception=(
            lambda e: isinstance(e, exception.BMCRedfishConnectionError)),
    ...
    def _get_system():
        try:
            ...
            raise exception.BMCRedfishError(error=e)
        ...
        # retrying on them
            ...
            raise exception.BMCRedfishConnectionError(node=node.uuid, error=e)

    try:
        return _get_system()
    except exception.BMCRedfishConnectionError as e:
        try:
            # try again as the session may timeout
            _CONN.pop(node_uuid)
            return _get_system()
        except exception.BMCRedfishConnectionError as e:
